...
class LocalExecutor:
    def __init__(self, data_loader: DataLoader):
        ...
        self.function_map = {
            ...
            "generate_stops_map": self.generate_stops_map,
            ...
            "plot_daily_kpi_trends": self.visualizer.plot_daily_kpi_trends,
            "plot_month_comparison": self.visualizer.plot_month_comparison,
            "plot_quarterly_comparison": self.visualizer.plot_quarterly_comparison,
            # Advanced querying functions
            "query_multi_month_data": self.query_multi_month_data,
            "aggregate_monthly_data": self.aggregate_monthly_data,
    ...
    def get_stop_accessibility(self, zone_id: str = None) -> pd.DataFrame:
        """Count stops by zone, optionally filter by specific zone."""
        stops = self.data_loader.get_stops()

        if "zone_id" in stops.columns:
            if zone_id:
                # Convert zone_id to integer (remove leading zeros if present)
                try:
    ...
    def generate_route_map(
        ...
        # Limit to max_routes unique routes (only if route_ids not specified)
        if not route_ids:
    ...
    def generate_stops_map(
        ...
        if "zone_id" in stops.columns:
            ...
            # No zone info, plot all stops with same color
            fig.add_trace(
                go.Scattermapbox(
                    lat=stops["stop_lat"],
                    lon=stops["stop_lon"],
    ...
    def get_top_routes_by_kpi(
        ...
        Args:
            ...
            months: Optional month(s) filter - can be a single month or list of months (e.g., ["August 2025"] or ["July 2025", "August 2025"])
        ...
        """
        # Load data for specific months if provided
    ...
    def aggregate_monthly_data(
        ...
        """
        # Use totals summary for service-level aggregations
        if service:
            df = self.data_loader.get_totals_summary(months=months)
        else:
            df = self.data_loader.get_monthly_data(months=months)
